setInterval(function () {
    ...
    const columns = document.querySelectorAll(
        '.PlannerRoot .planTaskBoardPage .taskBoardColumn'
    )
    columns.forEach(function (column) {
        let columnTitleSection = column.querySelector(
            '.columnHeader .titleSection'
        )
            ...
            while (i < 20 && matchResult !== null) {
                if (matchResult.length === 4) {
                    ...
                    if (typeContent) {
                        ...
                        cardTitle =
                            cardTitle.slice(0, matchResult.index) +
                            cardTitle.slice(
                                matchResult.index +
                                    matchResult[matchIndex].length +
                                    2
                            )
                    }
                }
        ...
        column
            .querySelectorAll('div.extraRow span.label-info')
            .forEach(function (it) {
        ...
        sumColumnRemainingEstimate =
            Math.round(sumColumnRemainingEstimate * 100) / 100

        let sumColumnOriginalEstimate = 0
        let hasOriginalSum = false
        column
            .querySelectorAll('div.extraRow span.label-default')
            .forEach(function (it) {
                let value = it.getAttribute('data-value')
                if (value) {
        ...
        sumColumnOriginalEstimate =
            Math.round(sumColumnOriginalEstimate * 100) / 100

        let sumColumnInvestedEstimate = 0
        let hasInvestedSum = false
        column
            .querySelectorAll('div.extraRow span.label-success')
            .forEach(function (it) {
                let value = it.getAttribute('data-value')
                if (value) {
                    sumColumnInvestedEstimate += parseFloat(value)
                    hasInvestedSum = true
                }
            })
        sumColumnInvestedEstimate =
            Math.round(sumColumnInvestedEstimate * 100) / 100
        ...
        if (hasInvestedSum) {
            let investedEstimateEl = document.createElement('div')
            investedEstimateEl.classList.add('label')
            investedEstimateEl.classList.add('label-success')
            investedEstimateEl.style.margin = '3px'
            investedEstimateEl.textContent = sumColumnInvestedEstimate
            sumHtml.appendChild(investedEstimateEl)
        }

        const estimatesPluginDiv = columnTitleSection.querySelector(
            '.colEstimatesPlugin'
        )
